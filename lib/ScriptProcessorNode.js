...
class ScriptProcessorNode extends AudioNode {
	...
	/**
	 * @param {(event: AudioProcessingEvent) => void} onaudioprocess
	 */
	set onaudioprocess(onaudioprocess) {
		let inputBuffer = new AudioBuffer(this.channelCount, 0, this.context.sampleRate);
		let outputBuffer = new AudioBuffer(this.numberOfOutputChannels, 0, this.context.sampleRate);
		...
		this._tick = () => {
			...
			} else if (inputBuffer.length >= this.bufferSize) {
...
class AudioProcessingEvent {
	 ...
	 * @param {import("./AudioBuffer")} inputBuffer
	 * @param {import("./AudioBuffer")} outputBuffer
	 */
